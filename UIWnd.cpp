...
LRESULT CALLBACK UIWnd::UIWndProc(HWND hWnd,
                                  UINT msg,
                                  WPARAM wParam,
                                  LPARAM lParam) {
  UIWnd* pUIWnd = NULL;
  if (WM_NCCREATE == msg) {
    ...
    if (pUIWnd == NULL)
      return FALSE;
    pUIWnd->m_hWnd = hWnd;
    SetWindowLongPtr(hWnd, GWLP_USERDATA, (LONG_PTR)pUIWnd);
  } else {
    pUIWnd = (UIWnd*)GetWindowLongPtr(hWnd, GWLP_USERDATA);
  }
...
HIMC UIWnd::GetHimc() {
  HIMC hImc = (HIMC)GetWindowLongPtr(m_hWnd, IMMGWL_IMC);
  // if (hImc == NULL) {
...
LRESULT CALLBACK UIWnd::WndProc(HWND hWnd,
                                UINT msg,
                                WPARAM wParam,
                                LPARAM lParam) {
  // HIMC hImc = GetHimc();
...
void UIWnd::RegisterUIWndClass(HINSTANCE hInstance) {
  ...
  wc.cbClsExtra = 0;
  wc.cbWndExtra = 2 * sizeof(LONG_PTR);  // must this size, for system use.
                                         // IMMGWL_IMC and IMMGWL_PRIVATE
...
void UIWnd::OnComposition(WPARAM wParam, LPARAM lParam) {
  ...
  if (lParam & GCS_COMPSTR) {
    mCompWnd.UpdateWindowPos(imcHandle);
    ::RedrawWindow(mCompWnd.GetWnd(), NULL, NULL,
